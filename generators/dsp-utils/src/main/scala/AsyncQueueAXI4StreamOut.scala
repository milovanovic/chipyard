...
//  val lazyDut = LazyModule(new AsyncQueueAXI4StreamOut(params) with AsyncQueueAXI4StreamOutStandalone)
//  (new ChiselStage).execute(Array("--target-dir", "verilog/AsyncQueueAXI4StreamOut"), Seq(ChiselGeneratorAnnotation(() => lazyDut.module)))
//}
//
//
//
