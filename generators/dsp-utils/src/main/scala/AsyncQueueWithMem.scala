...
//
//// This is a fast solution how to make AsyncQueue which uses block ram memory for it's implementation. Mem in this case is mapped to block ram
//
//// ClockCrossingReg - it seems like this one is not visible inside rocketchip version used inside rocket-dsptools snapshoot
...
//}
//
//case class AsyncQueueWithMemParams(
//  depth:  Int     = 8,
//  sync:   Int     = 3,
//  safe:   Boolean = true,
...
//}
//
//object AsyncQueueWithMemParams {
//  // When there is only one entry, we don't need narrow.
//  def singleton(sync: Int = 3, safe: Boolean = true) = AsyncQueueWithMemParams(1, sync, safe, false)
//}
//
//// imported from rockechip
...
//// }
//
//class AsyncBundle[T <: Data](private val gen: T, val params: AsyncQueueWithMemParams = AsyncQueueWithMemParams()) extends Bundle {
...
//// }
//
//class AsyncQueueWithMemSource[T <: Data](gen: T, params: AsyncQueueWithMemParams = AsyncQueueWithMemParams()) extends Module {
//  val io = IO(new Bundle {
...
//  val mem = Mem(params.depth, gen) //Reg(Vec(params.depth, gen)) //Mem(params.depth, gen) //Reg(Vec(params.depth, gen)) // This does NOT need to be reset at all.
...
//    //    assert (!(reset || !sio.sink_reset_n) || !io.enq.valid, "Enqueue while sink is reset and AsyncQueueWithMemSource is unprotected")
//    //    assert (!reset_rise || prev_idx_match.asBool, "Sink reset while AsyncQueueWithMemSource not empty")
//  }
//}
//
//class AsyncQueueWithMemSink[T <: Data](gen: T, params: AsyncQueueWithMemParams = AsyncQueueWithMemParams()) extends Module {
//  val io = IO(new Bundle {
...
//    // assert (!reset_rise || prev_idx_match.asBool, "Source reset while AsyncQueueWithMemSink not empty")
//  }
//}
//
...
////     val sink = Module(new AsyncQueueWithMemSink(chiselTypeOf(x.mem(0)), x.params.copy(sync = sync)))
////     sink.io.async <> x
////     sink.io.deq
////   }
//// }
...
//// {
////   def apply[T <: Data](x: ReadyValidIO[T], params: AsyncQueueWithMemParams = AsyncQueueWithMemParams()): AsyncBundle[T] = {
////     val source = Module(new AsyncQueueWithMemSource(chiselTypeOf(x.bits), params))
////     source.io.enq <> x
////     source.io.async
////   }
//// }
//
//class AsyncQueueWithMem[T <: Data](gen: T, params: AsyncQueueWithMemParams = AsyncQueueWithMemParams()) extends Crossing[T] {
//  val io = IO(new CrossingIO(gen))
//  val source = Module(new AsyncQueueWithMemSource(gen, params))
//  val sink   = Module(new AsyncQueueWithMemSink  (gen, params))
//
