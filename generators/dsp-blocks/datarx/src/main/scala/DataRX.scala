...
import dspblocks.utils.common.{
  AXI4StreamAsyncQueueWithControlBlock,
  AXI4StreamAsyncQueueWithControlParams,
  AXI4StreamAsyncQueueWithControlStandalone
}
...
case class DataRXParams(
  channels: Int = 4,
  asyncParams: Option[AXI4StreamAsyncQueueWithControlParams] = Some(AXI4StreamAsyncQueueWithControlParams(ctrlBits = 3,
      sync = 4, depth = 16, safe = true))) {
...
class DataRX(val params: DataRXParams) extends LazyModule()(Parameters.empty) {
  ...
  val asyncQueue =
    if (params.asyncParams.isDefined)
      Some(
        LazyModule(
          new AXI4StreamAsyncQueueWithControlBlock(params.asyncParams.get)
            with AXI4StreamAsyncQueueWithControlStandalone {
  ...
  // StreamNode
  val streamNode: AXI4StreamMasterNode = AXI4StreamMasterNode(
    AXI4StreamMasterParameters(name = "outStream", n = params.channels * 2, u = 0, numMasters = 1)
  )

  lazy val module = new LazyModuleImp(this) {
    val childClock = clock
    val childReset = reset
    ...
    word_detector.ioBlock.i_en := withClockAndReset(childClock, childReset) {
      ShiftRegister(bitslip.ioBlock.o_en, 3, 0.U, true.B)
    }
    // alligned data to word
    ...
    if (asyncQueue.isDefined) {
      ...
      io.o_crc := asyncQueue.get.pins.out_ctrl(
        byte2word.ioBlock.o_crc.getWidth + word_detector.ioBlock.o_word_size.getWidth - 1
      )
...
object DataRXwithAsyncApp extends App {
  ...
  (new ChiselStage)
    .execute(Array("--target-dir", "verilog/DataRXAsync"), Seq(ChiselGeneratorAnnotation(() => lazyDut.module)))
}
...
object DataRXwithoutAsyncApp extends App {
  ...
  val lazyDut = LazyModule(new DataRX(params) with DataRXPins)
  (new ChiselStage)
    .execute(Array("--target-dir", "verilog/DataRX"), Seq(ChiselGeneratorAnnotation(() => lazyDut.module)))
}
