        ...
        when(io.i_en) {
          when(counter === 0.U) {
            ...
            .elsewhen(counter === 1.U) {
              ...
              r_temp_data.zipWithIndex.foreach({
                case (m, i) =>
...
object Byte2WordApp extends App {
  ...
  (new ChiselStage)
    .execute(Array("--target-dir", "verilog/Byte2Word"), Seq(ChiselGeneratorAnnotation(() => lazyDut.module)))
}
