...
trait AXI4StreamAsyncQueueWithControlStandalone extends AXI4StreamAsyncQueueWithControlBlock {
  ...
  val ioInNode = BundleBridgeSource[AXI4StreamBundle](() => new AXI4StreamBundle(AXI4StreamBundleParameters(n = beatBytes)))(valName = ValName("input"))
  ...
  val in: ModuleValue[AXI4StreamBundle] = InModuleBody {
    ioInNode.makeIO()
  }
  val out: ModuleValue[AXI4StreamBundle] = InModuleBody {
    ioOutNode.makeIO()
  }
  def makeCustomIO() = {
